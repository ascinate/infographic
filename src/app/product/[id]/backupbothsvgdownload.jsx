...
const handleDownloadSvg = () => {
  const bgSvg = backgroundRef.current?.querySelector('svg');
  const fgSvg = foregroundRef.current?.querySelector('svg');
  const container = svgRef.current;

  if (!bgSvg || !fgSvg || !container) {
    alert("SVG layer not found");
    return;
  }
  ...
  const selected = parseInt(downloadSize, 10) || baseWidth;

  // We'll preserve aspect ratio: width = selected, height = selected / aspect
  const finalWidth = selected;
  const finalHeight = selected / aspectRatio;

  // Scale factor
  const scale = selected / baseWidth;

  // Clone SVGs
  const bgClone = bgSvg.cloneNode(true);
  const fgClone = fgSvg.cloneNode(true);
  bgClone.removeAttribute("width");
  bgClone.removeAttribute("height");
  fgClone.removeAttribute("width");
  fgClone.removeAttribute("height");

  // Create wrapper
  const wrapper = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  wrapper.setAttribute("xmlns", "http://www.w3.org/2000/svg");
  wrapper.setAttribute("width", finalWidth);
  wrapper.setAttribute("height", finalHeight);
  wrapper.setAttribute("viewBox", `0 0 ${baseWidth} ${baseHeight}`);
  ...
  const link = document.createElement('a');
  link.href = url;
  link.download = `combined-${finalWidth}x${Math.round(finalHeight)}.svg`;
            ...
            <div className="col-lg-8 ">
              ...
              <div className='canvas-editor'>
                ...
                >
                  ...
                  >
                    {backgroundType === 'simple' ? simpleBackground : detailedBackground}
                  </div>
              ...
              <div className="right-toolbar">
                ...
                <div className="download">
                  ...
                  <div>
                    ...
                    <button onClick={() => {
                      console.log('BG Ref:', backgroundRef.current?.innerHTML);
                      console.log('FG Ref:', foregroundRef.current?.innerHTML);
                    }}>
                      Check SVG Presence
                    </button>
